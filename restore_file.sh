...
read -p "Enter the file name to restore: " filename_to_restore
...
duplicity restore --no-encryption --no-print-statistics --s3-use-new-style -v 4 -t "$restore_date" \
  --file-to-restore "public_html/$filename_to_restore" \
  "$(awk -F'[="]' '/S3_url/ {print $3}' /root/.duplicity)/apps/$app_dir" \
  "/home/master/applications/$app_dir/tmp/$filename_to_restore"

# Echo the restored directory location
echo "The filename has been restored to: /home/master/applications/$app_dir/tmp/$filename_to_restore"
